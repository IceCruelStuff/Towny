...
class TownyFactory
{
    ...
    public function __construct(TownyLoader $plugin)
    {
        ...
        foreach (array_diff(scandir($plugin->getDataFolder() . "towns/"), [".", ".."]) as $file) {
            ...
            foreach ($towny->getVillagers() as $villager) {
                if (isset($this->playerToTowny[$villager])) {
                    unset($this->playerToTowny[$villager]); // if villager of another town
                } elseif (isset($this->playerToTowny[$villager]) && ((!$this->playerToTowny[$villager] instanceof Towny) || !$this->playerToTowny[$villager]->isVillager($villager))) {
                    unset($this->playerToTowny[$villager]); // if villager of another town and is not villager of town
                }
    ...
    public function getTownyByXZ(float $x, float $z, string $world) : ?Towny
    {
        foreach (array_values($this->towns) as $towny) {
            if (($towny->getStart()->getX() <= $x && $towny->getEnd()->getX() >= $x) &&
                ($towny->getStart()->getZ() <= $z && $towny->getEnd()->getZ() >= $z) &&
                $towny->getSpawn()->getWorld()->getFolderName() === $world) {
                return $towny;
            }
        }
        return null;
